...
@section('content')
    <main class="freshman-position">
        ...
        <form action="{{ route('senior.junior') }}" method="POST">
            @csrf
            @include('university.select.sophomore')
            <div class="text-right submit-btn-position">
                <div class="d-inline-block w-100">
                    <button type="submit" id="submit-btn" class="btn btn-secondary py-3 submit-btn">３年次へ</button>
                </div>
            </div>
        </form>
    </main>
